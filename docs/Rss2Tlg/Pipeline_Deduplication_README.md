...
**Версия:** 2.2 - Category Scoring & Importance Threshold  
**Дата:** 2025-11-10
...
DeduplicationService — второй этап AI Pipeline для обработки новостей. Модуль проверяет новости на наличие дубликатов среди уже опубликованных материалов, используя **двухэтапную проверку**: быструю предварительную фильтрацию и AI анализ.

### ✨ Новое в v2.2: Category Scoring & Importance Threshold

**Улучшения:**

1. **Category-based Similarity Scoring** 🎯
   - Добавлен категорийный скоринг в preliminary similarity
   - Primary == Primary: 100%, Primary == Secondary: 50%, Secondary overlap: 25%
   - Конфигурируемые веса: entities 30%, event 25%, keywords 20%, categories 25%
   - ↓15-20% ложных дубликатов (cross-category false positives)

2. **Importance Threshold** 🚀
   - Пропуск дедупликации для низкоприоритетных новостей
   - Параметр `min_importance_threshold` (по умолчанию 5)
   - ↓30-40% токенов, ↓25-35% времени обработки

3. **Двухэтапная проверка** (v2.1)
   - Быстрая текстовая проверка (preliminary similarity)
   - AI анализ только для подозрительных новостей
   - ↓80% токенов, ↓75% времени, ↓80% стоимости

### Основные функции

1. **Предварительная фильтрация** — быстрая проверка схожести (Jaccard + Cosine)
2. **AI-анализ схожести** — сравнение новостей через LLM (только топ-N)
3. **Сравнение сущностей** — проверка совпадения ключевых людей, организаций, мест
4. **Сравнение событий** — анализ core event новости
5. **Сравнение фактов** — проверка дат, чисел, конкретных данных
6. **Определение публикуемости** — решение можно ли публиковать новость
...
$config = [
    ...
    // v2.2: Importance filter
    'min_importance_threshold' => 5,          // Минимальная важность для дедупликации (0-20)
    
    // v2.2: Similarity weights
    'similarity_weights' => [
        'entities' => 30.0,                   // Вес схожести сущностей
        'event' => 25.0,                      // Вес схожести события
        'keywords' => 20.0,                   // Вес схожести ключевых слов
        'categories' => 25.0,                 // Вес схожести категорий (v2.2)
    ],
    
    // Preliminary similarity filter (v2.1)
    'max_preliminary_comparisons' => 50,      // Макс. новостей из БД для preliminary check
    'preliminary_similarity_threshold' => 60, // Порог для фильтрации (0-100)
    'max_ai_comparisons' => 10,               // Макс. новостей для AI анализа
    
    // AI analysis
    'similarity_threshold' => 70.0,           // Порог дубликата от AI (0-100)
    'compare_last_n_days' => 7,               // Период сравнения в днях
    'models' => [
...
//     // v2.1 метрики:
//     'preliminary_checks' => 450,         // всего preliminary проверок
//     'preliminary_filtered' => 380,       // отфильтровано (< threshold)
//     'ai_skipped' => 3,                   // новостей пропущено AI анализом
//     // v2.2 метрики:
//     'skipped_low_importance' => 4,       // пропущено по низкой важности
//     'model_attempts' => [...]
// ]
```
...
| **v2.2 Importance Filter** | | | |
| `min_importance_threshold` | int | 5 | Минимальная важность для дедупликации (0-20) |
| **v2.2 Similarity Weights** | | | |
| `similarity_weights.entities` | float | 30.0 | Вес схожести сущностей (0-100) |
| `similarity_weights.event` | float | 25.0 | Вес схожести события (0-100) |
| `similarity_weights.keywords` | float | 20.0 | Вес схожести ключевых слов (0-100) |
| `similarity_weights.categories` | float | 25.0 | Вес схожести категорий (0-100) |
| **v2.1 Preliminary Filter** | | | |
| `max_preliminary_comparisons` | int | 50 | Максимум новостей из БД для preliminary check |
| `preliminary_similarity_threshold` | float | 60.0 | Порог для фильтрации (0-100) |
| `max_ai_comparisons` | int | 10 | Максимум новостей для AI анализа |
| **AI Analysis** | | | |
| `similarity_threshold` | float | 70.0 | Порог схожести для дубликата от AI (0-100) |
