...
**Версия:** 2.1 - Preliminary Similarity Filter  
**Дата:** 2025-11-10
...
### ✨ Новое в v2.1: Preliminary Similarity Filter

**Оптимизация:** Теперь модуль использует **двухэтапную проверку** для снижения стоимости и ускорения работы:

1. **Этап 1: Быстрая текстовая проверка** (preliminary similarity)
   - Вычисление схожести через Jaccard + Cosine similarity
   - Использование билингвальных полей (_en) для кросс-языковой дедупликации
   - Фильтрация новостей с низкой схожестью (< 60%)
   - Отбор топ-N наиболее похожих для AI

2. **Этап 2: AI анализ** (только для подозрительных)
   - Глубокий анализ через LLM (DeepSeek, Gemma)
   - Только для новостей прошедших предварительный фильтр
   - Определение точной схожести и возможности публикации

**Результат:** ↓80% токенов, ↓75% времени, ↓80% стоимости
