...
CREATE TABLE `rss2tlg_deduplication` (
  ...
  `preliminary_similarity_score` decimal(5,2) DEFAULT NULL COMMENT 'Предварительная оценка схожести (0.00-100.00)',
  `similarity_method` enum('ai','hash','hybrid','preliminary') DEFAULT NULL COMMENT 'Метод определения схожести',
  `preliminary_method` varchar(50) DEFAULT 'hybrid_v1' COMMENT 'Метод предварительной оценки (hybrid_v1, jaccard, etc.)',
