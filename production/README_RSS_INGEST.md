...
При первом запуске или если в БД нет активных лент, скрипт автоматически:

1. Проверяет наличие активных лент: `SELECT COUNT(*) FROM rss2tlg_feeds WHERE enabled = 1`
2. Если лент нет - загружает `configs/feeds.json`
3. Синхронизирует данные:
   - Для существующих лент (по feed_url) - обновляет name, website_url, enabled
   - Для новых лент - создает записи
...
**Способ 1: Через feeds.json (рекомендуется при первичной настройке)**
1. Измените `enabled: false` в `configs/feeds.json`
2. Очистите таблицу: `TRUNCATE TABLE rss2tlg_feeds;`
3. Перезапустите скрипт

**Способ 2: Напрямую в БД (рекомендуется для оперативных изменений)**
```sql
-- Отключить источник
