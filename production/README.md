...
│   ├── ai_pipeline.json               # AI модули (NEW)
│   └── feeds.json                     # RSS источники (справочно)
├── sql/                                # SQL дампы и схемы
│   ├── rss2tlg_feeds_dump.sql
│   ├── rss2tlg_items_dump.sql
│   ├── statistics_schema.sql          # Схема статистики (NEW)
│   └── ... (10 файлов)
├── rss_ingest.php                     # Скрипт сбора RSS
├── ai_summarization.php               # Скрипт AI суммаризации (NEW)
├── run_3_tests.sh                     # Тест RSS: 3 запуска с интервалом 2 мин
├── run_3_tests_fast.sh                # Быстрый тест RSS: 3 запуска за 30 сек
├── test_ai_summarization.sh           # Тест AI: 3 запуска с интервалом 1 мин (NEW)
├── setup_cron.sh                      # Настройка cron для RSS
├── setup_ai_summarization_cron.sh     # Настройка cron для AI (NEW)
├── TEST_REPORT.md                     # Отчет о тестировании RSS
...
## 🤖 Скрипт: ai_summarization.php

### Описание

Production скрипт для AI суммаризации и категоризации собранных новостей.
...
- ✅ Поддержка fallback между моделями (Claude 3.5 Sonnet → DeepSeek Chat)
...
```php
const TEST_MODE = false; // Обрабатывать все необработанные новости
```
...
|--------|-----------|-------------------|
| Claude 3.5 Sonnet | Primary | $3/$15 (input/output) |
| DeepSeek Chat | Fallback | $0.14/$0.28 |

**Экономия от кеширования:** ~75% на повторных запросах (Claude)

### Конфигурационные файлы

| Файл | Назначение |
|------|------------|
| `configs/ai_pipeline.json` | Настройки AI модулей, модели, промпты |
| `configs/database.json` | Подключение к БД |
| `configs/telegram.json` | Telegram бот (token, chat_id) |

### Логи

- **Основной лог:** `/home/engine/project/logs/ai_summarization.log`
- **Cron лог:** `/home/engine/project/logs/cron_ai_summarization.log`
- **Тестовый лог:** `/home/engine/project/logs/test_ai_summarization.log`

### Статистика

Все метрики сохраняются в БД:

- Таблица `rss2tlg_statistics` - статистика по каждому запуску
- Таблица `rss2tlg_daily_summaries` - ежесуточные сводки
- View `v_rss2tlg_statistics_daily` - агрегация по дням

### Ежесуточная сводка

Отправляется автоматически в **00:00 MSK** (21:00 UTC) со следующими данными:

- 🚀 Количество запусков за сутки
- 📰 Статистика обработки (всего/успешно/ошибок/пропущено)
- 🎯 Использование токенов (всего/prompt/completion/cached)
- 💰 Стоимость использования API с разбивкой по моделям
- ⏱ Производительность (время обработки)
- 🤖 Распределение по моделям
- 📈 Топ-5 категорий
- 🌍 Распределение по языкам
- ⭐ Статистика важности новостей
...
### Настройка cron (каждую 1 минуту)
...
```bash
# Запуск теста (3 запуска с интервалом 1 минута)
...
```bash
# Последние записи
tail -100 logs/ai_summarization.log

# В реальном времени
tail -f logs/ai_summarization.log

# Только ошибки
grep ERROR logs/ai_summarization.log

# Финальные отчеты
grep -A 20 "ФИНАЛЬНЫЙ ОТЧЕТ" logs/ai_summarization.log
```
...
### run_3_tests.sh (RSS Ingest)

Запускает **rss_ingest.php** 3 раза с интервалом **2 минуты** (реальный cron интервал).
...
### run_3_tests_fast.sh (RSS Ingest)

Запускает **rss_ingest.php** 3 раза с интервалом **10 секунд** (для быстрого тестирования).
...
### test_ai_summarization.sh (AI Summarization)

Запускает **ai_summarization.php** 3 раза с интервалом **60 секунд** (реальный cron интервал).

```bash
./production/test_ai_summarization.sh
```

**Время выполнения:** ~3 минуты (зависит от количества новостей и скорости AI)

**Что тестируется:**
- Обработка через AI модуль суммаризации
